        ...
        <div className="fixed top-0 left-0 right-0 z-50 flex flex-col items-center pt-[14px] px-[12px]">
          ...
          <form onSubmit={handleSearch} className="w-full max-w-[640px]">
            <div className="flex h-[48px] items-center rounded-full bg-white shadow-[0_8px_32px_rgba(0,0,0,0.12)] px-[20px]">
              {isSearching ? (
                <Loader2 className="h-[20px] w-[20px] shrink-0 text-[var(--color-gray-400)] animate-spin" />
              ) : (
                <Search className="h-[20px] w-[20px] shrink-0 text-[var(--color-gray-400)]" />
              )}
              <input
                type="search"
                value={searchQuery}
                ...
                className="flex-1 bg-transparent px-[16px] text-[15px] text-[var(--color-gray-900)] placeholder-[var(--color-gray-400)] outline-none border-none focus:ring-0"
              />
              {searchQuery && (
                <button
                  type="button"
                  onClick={handleClearSearch}
                  className="flex h-[24px] w-[24px] items-center justify-center rounded-full hover:bg-[var(--color-bg-input)] transition-colors"
                >
                  <X className="h-[16px] w-[16px] text-[var(--color-gray-400)]" />
                </button>
              )}
              ...
              {searchQuery.trim().length >= 2 && (
                <button
                  type="submit"
                  className="flex h-[32px] w-[32px] items-center justify-center rounded-full hover:bg-[var(--color-bg-input)] transition-colors"
          ...
          {showResults && (
            <div className="w-full max-w-[640px] mt-[8px] bg-white rounded-[20px] shadow-[0_8px_32px_rgba(0,0,0,0.12)] overflow-hidden max-h-[70vh] overflow-y-auto">
              {isSearching && !hasResults ? (
                ...
                <div className="divide-y divide-[var(--color-bg-input)]">
                ...
                <div className="flex flex-col items-center justify-center py-[32px] text-[var(--color-gray-400)]">
      ...
      <header className="sticky top-0 mx-[12px] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.08)] rounded-b-[20px] z-30">
        <div className="flex h-[76px] items-center justify-between px-[24px]">
          ...
          <div className="flex items-center shrink-0">
            ...
            <button
              onClick={onMenuClick}
              className={cn(
                "flex h-[40px] w-[40px] items-center justify-center rounded-full hover:bg-[var(--color-bg-input)] lg:hidden transition-colors",
                isRTL ? "ml-[8px]" : "mr-[8px]"
              )}
              aria-label={t('common.openMenu')}
            >
              <Menu className="h-[24px] w-[24px] text-[var(--color-gray-900)]" />
            </button>
          ...
          >
            <div className="flex h-[48px] w-full max-w-[640px] items-center rounded-full bg-[var(--color-bg-input)] px-[20px]">
              <Search className="h-[20px] w-[20px] shrink-0 text-[var(--color-gray-400)]" />
              <input
                type="search"
                value={searchQuery}
          ...
          <div className="flex items-center gap-[10px] shrink-0">
            ...
            {isAuthenticated ? (
              ...
              <div className="flex items-center gap-[8px]">
                <Link
                  href="/profile"
                  className="flex h-[48px] items-center gap-[10px] rounded-full bg-[var(--color-bg-input)] px-[20px] text-[var(--color-gray-900)] transition-colors hover:bg-[var(--color-gray-200)]"
                >
                  <div
                ...
                <button
                  onClick={logout}
                  className="flex items-center justify-center w-[40px] h-[40px] rounded-full hover:bg-[var(--color-bg-input)] transition-colors"
                  title={t('common.logout')}
                >
                  <LogOut className="h-[18px] w-[18px] text-[var(--color-gray-500)]" strokeWidth={2} />
                </button>
              </div>
            ) : (
        ...
        <div className="px-[24px] pb-[16px] md:hidden">
          <form onSubmit={handleSearch}>
            <div className="flex h-[44px] items-center rounded-full bg-[var(--color-bg-input)] px-[18px]">
              <Search className="h-[18px] w-[18px] shrink-0 text-[var(--color-gray-400)]" />
              <input
                type="search"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                onFocus={() => setSearchFocused(true)}
                placeholder={t('common.searchPlaceholder')}
                className="flex-1 bg-transparent px-[14px] text-[15px] text-[var(--color-gray-900)] placeholder-[var(--color-gray-400)] outline-none border-none focus:ring-0"
              />
            </div>
          </form>
        </div>
...
function SearchResultItem({
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-[12px] p-[12px] hover:bg-[var(--color-gray-50)] transition-colors cursor-pointer",
        isRTL && "flex-row-reverse"
      )}
    >
      {/* Product Image */}
      <div className="w-[60px] h-[60px] rounded-[12px] bg-[var(--color-bg-input)] overflow-hidden flex-shrink-0">
      ...
      <div className={cn("flex-1 min-w-0", isRTL && "text-right")}>
        <p className="text-[14px] font-medium text-[var(--color-gray-900)] line-clamp-1">
          {displayName}
        </p>
      ...
      <div className="flex-shrink-0">
        {isOutOfStock ? (
          <span className="text-[12px] text-[var(--color-gray-400)] px-[12px] py-[8px] bg-[var(--color-bg-input)] rounded-full">
            {isRTL ? 'نفذ' : 'Out'}
          </span>
        ) : cartQuantity > 0 ? (
          /* Quantity Stepper */
